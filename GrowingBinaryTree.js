function GrowingBinaryTree() {
    ...
    this.setup = () => {
        ...
        this.activeSet.push(this.cells[i][j]);
    }
    this.loop = () => {
        if (this.activeSet.length === 0) return;
        // TODO: Handle Criteria

        // For random Pick
        let r = floor(this.activeSet.length * random());

        // For Newest
        r = this.activeSet.length - 1;
        // For Oldest
        // r = 0;
        const x = this.activeSet[r];
        x.visited = true;
        let n = this.getNeighbours(x);
        if (n) {
            n.visited = true;
            x.removeWall(n);
            this.activeSet.push(n);
            n = this.getNeighbours(x);
            if (n) {
                n.visited = true;
                x.removeWall(n);
                this.activeSet.push(n);
